...
class MetricsService
{
    ...
    public function dump(): string
    {
        ...
        // Redis::keys() возвращает ключи С глобальным префиксом (e.g. "laravel-database-"),
        // а Redis::get() добавляет префикс повторно. Убираем префикс чтобы избежать дублирования.
        $redisPrefix = (string) config('database.redis.options.prefix', '');

        $metrics = [];

        foreach ($keys as $redisKey) {
            $strippedKey = $redisKey;
            if ($redisPrefix !== '' && str_starts_with($strippedKey, $redisPrefix)) {
                $strippedKey = substr($strippedKey, strlen($redisPrefix));
            }
            ...
            $parsed = $this->parseKey((string) $strippedKey);
            $metrics[$parsed['name']][] = [
