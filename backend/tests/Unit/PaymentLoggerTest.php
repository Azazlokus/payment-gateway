...
class PaymentLoggerTest extends TestCase
{
    ...
    public function test_info_logs_to_payments_channel(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with('Test message', \Mockery::on(fn ($ctx): bool => $ctx['service'] === 'payment-gateway'));
    ...
    public function test_enriched_context_contains_service_key(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with(\Mockery::any(), \Mockery::on(fn ($ctx): bool => $ctx['service'] === 'payment-gateway'
            ));
    ...
    public function test_enriched_context_contains_environment(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with(\Mockery::any(), \Mockery::on(fn ($ctx): bool => isset($ctx['environment'])
            ));
    ...
    public function test_enriched_context_contains_timestamp(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with(\Mockery::any(), \Mockery::on(fn ($ctx): bool => isset($ctx['timestamp']) && str_contains($ctx['timestamp'], 'T')
            ));
    ...
    public function test_caller_context_is_merged_into_enriched(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with('msg', \Mockery::on(fn ($ctx): bool => $ctx['payment_id'] === 'pay-123' &&
    ...
    public function test_caller_context_overrides_enriched_keys(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with('msg', \Mockery::on(fn ($ctx): bool => $ctx['service'] === 'custom-service'
            ));
    ...
    public function test_correlation_id_included_from_request_header(): void
    {
        ...
        Log::shouldReceive('info')
            ->once()
            ->with('msg', \Mockery::on(fn ($ctx): bool => $ctx['correlation_id'] === 'trace-abc-123'
            ));
